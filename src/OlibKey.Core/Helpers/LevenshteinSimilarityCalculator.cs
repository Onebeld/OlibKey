...
public static class LevenshteinSimilarityCalculator
{
	/// <summary>
	...
	private static int GetLevenshteinDistance(string source, string target)
	{
		...
		for (int sourceIndex = 0; sourceIndex < source.Length; sourceIndex++)
		{
			...
			for (int targetIndex = 0; targetIndex < target.Length; targetIndex++)
			{
				...
				vector1[targetIndex + 1] = MathHelpers.GetMinFromNumbers(vector1[targetIndex] + 1,
					vector0[targetIndex + 1] + 1, vector0[targetIndex] + cost);
			}
