...
public static class StorageProvider
{
	...
	{
		IReadOnlyList<IStorageFolder> result = await OlibKeyApp.TopLevel.StorageProvider.OpenFolderPickerAsync(
			new FolderPickerOpenOptions
			{
				SuggestedStartLocation = directory is null
					? null
					: await OlibKeyApp.TopLevel.StorageProvider.TryGetFolderFromPathAsync(new Uri(directory))
			});

		return result.Count == 0 ? null : result[0].Path.LocalPath;
	}

	public static async Task<string?> SelectFile(string? title = null,
		IReadOnlyList<FilePickerFileType>? pickerFileTypes = null)
	{
		IReadOnlyList<IStorageFile> result = await OlibKeyApp.TopLevel.StorageProvider.OpenFilePickerAsync(
