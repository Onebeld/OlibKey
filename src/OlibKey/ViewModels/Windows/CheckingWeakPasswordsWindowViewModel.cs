...
namespace OlibKey.ViewModels.Windows
{
    public class CheckingWeakPasswordsWindowViewModel : ReactiveObject
    {
        ...
        public CheckingWeakPasswordsWindowViewModel()
        {
            ...
            for (int index = 0; index < ((DatabaseControl)App.MainWindowViewModel.SelectedTabItem.Content).ViewModel.LoginList.Count; index++)
            {
                LoginListItem item = ((DatabaseControl)App.MainWindowViewModel.SelectedTabItem.Content).ViewModel.LoginList[index];
            ...
            sum += ((DatabaseControl)App.MainWindowViewModel.SelectedTabItem.Content).ViewModel.LoginList.Where(item => item.LoginItem.Type == 0)
            ...
            int count = ((DatabaseControl)App.MainWindowViewModel.SelectedTabItem.Content).ViewModel.LoginList.Count(item => item.LoginItem.Type == 0);
        ...
        private async void ChangeWeakPassword()
        {
            try
            {
                for (int index = 0; index < LoginList.Count; index++)
                {
                    ...
                    {
                        foreach (LoginListItem item in ((DatabaseControl)App.MainWindowViewModel.SelectedTabItem.Content).ViewModel.LoginList.Where(
